...
（すぐ試したいときは「再試行」）。切れたままの場合は間隔を最大5分まで伸ばします。

つながったかどうかの判定には、**どのキャッシュにも答えられない専用のリクエスト**を
使っています。ふつうの取得は、ブラウザのキャッシュ（`max-age`）が応えれば
サーバーが落ちていても成功するため、応答が返ったことを復帰の根拠にできません。
